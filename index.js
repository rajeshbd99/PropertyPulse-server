...
app.get('/property/:id', async (req, res) => {
  ...
  try {
    ...
    if (property) {
      res.json({ ...property, priceRange: property.priceRange || { min: 0, max: Infinity } });
    } else {
...
app.post('/make-offer', async (req, res) => {
  ...
  if (offerAmount < priceRange.min || offerAmount > priceRange.max) {
    return res
      .status(400)
      .json({ message: `Offer must be between $${priceRange.min} and $${priceRange.max}.` });
  }
  ...
  try {
    ...
    res.json({ message: 'Offer made successfully!', offerId: result.insertedId });
  } catch (error) {
